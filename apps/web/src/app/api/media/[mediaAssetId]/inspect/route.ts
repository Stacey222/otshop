...
import { getMediaInspectionService } from "@/infrastructure/media/runtime";

export const runtime = "nodejs";

export async function POST(
  request: NextRequest,
  routeContext: RouteContext<"/api/media/[mediaAssetId]/inspect">,
): Promise<Response> {
  return withApiHandler(async (canonicalRequest, { requestId }) => {
    ...
    const result = await getMediaInspectionService().inspect({
      context,
      requestId,
      mediaAssetId,
    });
