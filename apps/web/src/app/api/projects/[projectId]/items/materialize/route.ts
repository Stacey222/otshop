...
import { getProjectItemService } from "@/infrastructure/projects/project-item-runtime";

export const runtime = "nodejs";

export async function POST(
  request: NextRequest,
  routeContext: RouteContext<"/api/projects/[projectId]/items/materialize">,
) {
  return withApiHandler(async (canonicalRequest, { requestId }) => {
    ...
    const materialization = await getProjectItemService().materialize({
      context,
      requestId,
      projectId,
      body: await readBoundedJson(canonicalRequest, PROJECT_MAX_METADATA_BYTES),
    });
    return Response.json({ materialization }, { headers: { "Cache-Control": "no-store" } });
  })(request);
}
