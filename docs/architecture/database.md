...
`id uuid` PK; `workspace_id uuid` non-null; `source media_source` non-null (`LOCAL_FILE`, `MANUAL_UPLOAD`); `original_filename text` non-null sanitized for display only; `storage_key text` non-null; `mime_type text` non-null; `size_bytes bigint` non-null check > 0; `sha256 bytea` non-null check length 32; `status text` non-null; `duration_ms bigint`, `width integer`, `height integer`, `fps numeric`, `bitrate_bps bigint`, `codec text`, `audio_codec text`, `orientation text`, `thumbnail_key text` nullable until probed; `content_source`, `content_owner`, `license_reference`, `notes` nullable; `validation_error_code text` nullable; timestamps; `version integer` non-null. Slice 3.2 defines `INGESTED`, `INSPECTING`, `READY`, `REJECTED`, and `INSPECTION_FAILED`. READY atomically requires normalized duration, dimensions, frame rate, codec, and orientation; optional bitrate/audio remain nullable. Rejection/failure clears probe metadata and records a bounded validation code. Orientation is `ROTATION_0`, `ROTATION_90`, `ROTATION_180`, or `ROTATION_270`.
